...
# ---- Chart 2: Movie Count Per Genre (Top 10) ----
...
WHERE genres IS NOT NULL
"""
all_genres_df = run_query(all_genres_query)

if not all_genres_df.empty:
    # Split pipe-separated genres into individual rows
    genre_counts = {}
    for g in all_genres_df["genres"]:
        for genre in g.split("|"):
            genre = genre.strip()
            genre_counts[genre] = genre_counts.get(genre, 0) + 1

    genre_count_df = pd.DataFrame(
        list(genre_counts.items()), columns=["genre", "movie_count"]
    ).sort_values("movie_count", ascending=False).head(10)

    st.bar_chart(
        data=genre_count_df.set_index("genre"),
